...
-- Create app tables with unique names
...
CREATE TABLE app.user_delivery_selections (
  id uuid PRIMARY KEY REFERENCES auth.users(id) ON DELETE CASCADE,
...
-- Insert comprehensive delivery windows data (19 windows: weekdays have 3 slots, weekends have 2 slots)
...
BEGIN
  ...
  INSERT INTO app.user_delivery_staging (id, staged_address_label, staged_window_id, staged_at)
...
BEGIN
  ...
  -- Upsert user_delivery_selections
  INSERT INTO app.user_delivery_selections (id, selected_address_label, selected_window_id, week_start, confirmed_at)
  ...
  -- Upsert user_week_status
  INSERT INTO app.user_week_status (id, week_start, is_delivery_confirmed, confirmed_at, updated_at)
...
SELECT 
  ...
  u.created_at
FROM auth.users u
LEFT JOIN app.user_profiles up ON u.id = up.id;
...
LEFT JOIN app.user_delivery_selections us ON up.id = us.id
LEFT JOIN app.user_week_status uws ON up.id = uws.id;
...
FROM app.user_profile up
LEFT JOIN app.user_delivery_staging uds ON up.id = uds.id
LEFT JOIN app.user_delivery_selections us ON up.id = us.id
LEFT JOIN app.user_week_status uws ON up.id = uws.id;
...
ALTER TABLE app.user_delivery_selections ENABLE ROW LEVEL SECURITY;

-- RLS policies for app.user_profiles
CREATE POLICY "Users can view own profile" ON app.user_profiles FOR SELECT USING (auth.uid() = id);
CREATE POLICY "Users can update own profile" ON app.user_profiles FOR UPDATE USING (auth.uid() = id);
CREATE POLICY "Users can insert own profile" ON app.user_profiles FOR INSERT WITH CHECK (auth.uid() = id);

-- RLS policies for app.user_delivery_staging
CREATE POLICY "Users can manage own staging" ON app.user_delivery_staging FOR ALL USING (auth.uid() = id);

-- RLS policies for app.user_week_status
CREATE POLICY "Users can manage own week status" ON app.user_week_status FOR ALL USING (auth.uid() = id);

-- RLS policies for app.user_delivery_selections
CREATE POLICY "Users can manage own selections" ON app.user_delivery_selections FOR ALL USING (auth.uid() = id);
