-- Robust Migration Script for YeneFresh App
-- This migration handles all error scenarios and ensures idempotency
-- Run this single file to set up the entire database schema safely
...
CREATE TABLE public.orders (
  ...
  week_start date NOT NULL,
  window_id uuid NOT NULL REFERENCES public.delivery_windows(id),
  address jsonb NOT NULL, -- {name, phone, line1, line2, city, notes}
...
BEGIN
  ...
  SELECT COALESCE(uos.meals_per_week, 3)
    INTO v_meals
  FROM public.user_onboarding_state uos
  WHERE uos.user_id = v_user;

  -- Selected recipes for current week
  WITH sel AS (
...
GRANT EXECUTE ON FUNCTION app.confirm_scheduled_order(jsonb) TO authenticated;
