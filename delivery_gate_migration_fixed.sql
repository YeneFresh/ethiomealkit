-- Complete Delivery Gate Migration (Fixed)
...
BEGIN
  ...
  -- Upsert user_selections
  INSERT INTO app.user_selections (id, selected_address_label, selected_window_id, week_start, confirmed_at)
...
-- Create views (using the renamed table)
...
LEFT JOIN app.user_selections us ON up.id = us.id
...
ALTER TABLE app.user_selections ENABLE ROW LEVEL SECURITY;
...
-- RLS policies for app.user_selections
CREATE POLICY "Users can manage own selections" ON app.user_selections FOR ALL USING (auth.uid() = id);
