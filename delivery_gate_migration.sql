-- Complete Delivery Gate Migration
...
SELECT 
  ...
  u.created_at
FROM auth.users u
LEFT JOIN app.user_profile up ON u.id = up.id;
...
-- RLS policies for app.user_profile
CREATE POLICY "Users can view own profile" ON app.user_profile FOR SELECT USING (auth.uid() = id);
CREATE POLICY "Users can update own profile" ON app.user_profile FOR UPDATE USING (auth.uid() = id);
CREATE POLICY "Users can insert own profile" ON app.user_profile FOR INSERT WITH CHECK (auth.uid() = id);
